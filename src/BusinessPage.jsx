    ...
    <main style={{ background: "#F6F2FC", minHeight: "100dvh" }}>
      <div style={{ width: "min(960px, 100%)", margin: "0 auto", padding: "calc(22px + var(--yume-safe-top)) 20px calc(80px + var(--yume-safe-bottom))" }}>
        ...
        <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <button onClick={() => navigate(ROUTES.home)} style={{ border: "none", background: "transparent", cursor: "pointer", padding: 0, fontSize: 14, fontWeight: 700, letterSpacing: "0.22em", color: UI.accent }}>
            YUME
          </button>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <button onClick={() => navigate(ROUTES.app)} style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: 13.5, color: UI.ink2, padding: "8px 10px" }}>
              개인용으로 써보기
            </button>
            <a href="/docs/api" style={{ ...btnGhost, padding: "9px 16px", fontSize: 13.5 }}>API 문서</a>
          </div>
        </nav>
    ...
    "claims": [{
      ...
      "explanation": "행정규칙 검색공간의 약 85%를 탐색했지만 …"
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 18 }}>
            <a href="/docs/api" style={btn}>API 문서 보기</a>
            <button onClick={() => navigate(ROUTES.app)} style={btnGhost}>가입하고 키 발급받기</button>
          </div>
        </Section>

        {/* ── 업종별 ──────────────────────────────────────────────── */}
        <Section eyebrow="업종별 쓰임" title="어떤 실패를 막는지" lede="분야마다 AI가 무너지는 지점이 다릅니다.">
        ...
        <footer style={{ marginTop: 44, paddingTop: 20, borderTop: `1px solid ${UI.hairline}`, fontSize: 13, color: UI.ink3, display: "flex", gap: 14, flexWrap: "wrap" }}>
          <a href={`mailto:${CONTACT_EMAIL}`} className="yume-link" style={{ color: UI.ink3 }}>{CONTACT_EMAIL}</a>
