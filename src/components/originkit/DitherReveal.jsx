// Originkit hero-40의 Dither Reveal — WebGL 셰이더로 이미지를 디더 패턴으로 그리고,
// 포인터 주변만 원본 색으로 드러낸다. 셰이더가 이미 보라 계열로 리컬러링하고 있어
// 유메 팔레트와 그대로 맞는다. 원본은 TSX라 타입 구문만 걷어내고 로직은 손대지 않았다.
...
function __OriginkitBase_DitherReveal(props) {
  const {
...
void main() {
    ...
    vec3 ditherDark = vec3(0.20, 0.06, 0.38);
    vec3 ditherLight = vec3(0.73, 0.57, 0.93);
    ...
    float finalAlpha = mix(subjectAlpha * 0.58, subjectAlpha, revealAmount);
    gl_FragColor = vec4(mix(ditherColor, color.rgb, revealAmount), finalAlpha);
}
`;

// Originkit 프리셋. 원본은 이미지가 만료된 blob URL이라 public에 받아둔 파일로 바꿨다.
const __originkitPresetProps = {
  image: { src: "/originkit/hero-40/white-hands.png", alt: "" },
  dotSize: 7,
  revealRadius: 98,
  revealSoftness: 59,
};

export default function DitherReveal(props) {
  return <__OriginkitBase_DitherReveal {...__originkitPresetProps} {...props} />;
}
